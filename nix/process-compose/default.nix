  ...
  nginxConf = pkgs.writeText "nginx.conf" ''
    ...
    http {
      ...
      server {
        ...
        # The conditional redirect logic from your production config
        set $do_redirect "yes";
        if ($request_uri ~ ^/api/(v1/boot|setup/bootstrap/callback|setup/finalize|provisioning/files)) {
          set $do_redirect "no";
        }
        if ($do_redirect = "yes") {
          # Redirecting to our dev HTTPS port (8443)
          return 301 https://$host:8443$request_uri;
        }

        # If it hits the 'no redirect' condition, serve over HTTP
        location /api/ {
      ...
      server {
        ...
        # Pointing to the certs generated by Nix
