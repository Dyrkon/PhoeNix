...
// TODO migrate to entity and add to systems
public class SystemUser : AggregateRoot<SystemUserId>
{
    ...
    private SystemUser(SystemUserId id) : base(id)
    {
    }
    ...
    public static Result<SystemUser> Create(SystemUserId id, string name, string description, string group,
        bool isNormalUser, uint uid, Shell shell, string homePath)
    {
        return new SystemUser(id)
        {
            Name = name, Description = description, Group = group, IsNormalUser = isNormalUser, Shell = shell,
            HomePath = homePath, Uid = uid
        };
    }

    public Result<SystemUserBuildResult> Build()
    {
        ...
        return new SystemUserBuildResult($"{Name} = {{ description  = \"{Description}\"; group = \"{Group}\";" +
